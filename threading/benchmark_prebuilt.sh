...
DEBUG_OUTPUT_FILE="out_prebuilt_${CONFIG_TYPE}.txt"
...
echo "Starting $CONFIG_NAME benchmarks (pre-building once, then running $RUNS times without cleaning)..."
...
run_benchmark() {
    ...
    # Pre-build once before running benchmarks
    echo "Pre-building $config_name configuration..."
    dub clean > /dev/null 2>&1
    dub build --compiler=ldc2 --config=$config > /dev/null 2>&1
    if [ $? -ne 0 ]; then
        echo "Error: Failed to build $config_name configuration"
        return 1
    fi
    echo "Pre-build complete. Starting $RUNS runs..."
    ...
    # Run the benchmarks (no cleaning between runs)
    for i in $(seq 1 $RUNS); do
        ...
        # dub run will skip building if binary is already up-to-date
...
echo ""
echo "========================================="
echo "Final Benchmark Results: $CONFIG_NAME ($RUNS runs each, pre-built)"
echo "========================================="
echo ""
echo "Native Config:"
